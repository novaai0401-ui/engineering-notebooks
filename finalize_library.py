...
java_tests=sum(x['passed'] for x in junit)
assert java_tests==11
cards=[]
for book in report['books']:
    ...
    cards.append(f'<article class="card"><h2><a href="{stem}.html">{book["title"]}</a></h2><p>{chapters} lessons Â· {book["words"]:,} words</p><p><a href="{stem}.ipynb">Notebook</a> Â· <a href="{stem}.md">Source</a></p></article>')
...
summary=f'<h1>Your engineering learning library</h1><p>{len(report["books"])} notebooks Â· {lessons} lessons Â· {words:,} words Â· {count} passing notebook labs</p><p class="notice">Simple explanations lead to working code, failure cases, design choices and interview practice. Includes databases, six RAG routes, Kafka, caching, real-time delivery, Docker and Kubernetes. Read offline; running code requires the documented software. Recipes and verified executions are labelled separately.</p><p><a href="START-HERE.html">Learning route</a> Â· <a href="STUDY-ON-ANY-DEVICE.html">Laptop and mobile commands</a> Â· <a href="VALIDATION.html">Evidence and limitations</a> Â· <a href="COMPLETION-AUDIT.html">Completion audit</a></p><div class="cards">'+''.join(cards)+'</div>'
(root/'index.html').write_text(scope['page']('Engineering Notebook Library',summary),encoding='utf-8')
lines=['# Validation and coverage report','','Updated on 24 September 2026; individual reports include retained earlier evidence. Executable evidence is distinguished from deployment recipes and architectural extensions.','','## Results','','| Check | Result |','| --- | --- |',f'| Notebook execution | {count} passed; no failed cells |',f'| Reading material | 21 notebooks, {lessons} lessons, {words:,} words |',f'| Connected Java service | {java_tests} tests passed: 8 integration and 3 session-race component tests |','| Original Spring starter | 3 tests passed in foundation validation |','| React unit/DOM/hydration | 3 tests passed |','| Connected browser tests | 12 passed across Chromium, Firefox and WebKit, including failure journeys and axe checks |','| Python package | Wheel and source distribution built; strict mypy passed; 5 pytest cases passed |',f'| Remote MCP | {len(mcp["passed"])} network check groups passed |',f'| Full-stack HTTP/recovery | {len(system["passed"])} check groups passed |','| Docker/cloud | Docker unavailable; recipes supplied, not executed |','| Local model | Actual Ollama tool calling and generation exercised; see advanced evidence below |','','## Notebook contents','','| Notebook | Lessons | Passing executable cells |','| --- | --- | --- |']
for book in report['books']:
...
for relative in ['labs/rag-flow/bounded-generation-report.json','labs/rag-flow/development-regression-report.json','labs/rag-flow/generation-recovery-report.json','labs/rag-flow/graph-pipeline-report.json','labs/study-coach/durable-spring-initial-report.json','labs/study-coach/durable-spring-report.json','labs/realtime-cache/multi-instance-report.json','labs/linux-runtime-report.json','labs/docker-runtime-report.json','labs/k3s-install-report.json','labs/redis-failover-report.json','labs/study-coach/container-report.json','labs/kubernetes-report.json']:
    ...
    if candidate.exists():
        data=json.loads(candidate.read_text(encoding='utf-8'));report['projects'][relative]=data
        advanced+=['### '+relative,'','```json',json.dumps(data,indent=2),'```','']
for candidate in [root/'labs/study-coach/python/semantic-regression-report.json',root/'labs/study-coach/live-stack-buffered-report.json',root/'labs/study-coach/identity-session-race-report.json',root/'labs/study-coach/python/generation-boundary-report.json',root/'acceptance-status.json',root/'reading-accessibility-report.json',root/'labs/database-plan-workshop/mysql-report.json',root/'labs/database-plan-workshop/oracle-readiness-failure-report.json',root/'labs/database-plan-workshop/oracle-report.json',root/'labs/load-balancing/tls-websocket-report.json',root/'labs/study-coach/durable-spring-crash-report.json',root/'labs/study-coach/durable-spring-expiry-report.json',root/'labs/study-coach/secret-overlap-initial-report.json',root/'labs/study-coach/secret-overlap-report.json',root/'labs/rag-flow/expanded-evaluation-report.json',root/'labs/rag-flow/public-evaluation-report.json',root/'labs/load-balancing/test-report.json',root/'labs/ci-prior-report.json',root/'labs/interview-workshop/test-report.json',root/'labs/kubernetes-initial-report.json',root/'labs/kubernetes-initial-review.json',root/'labs/kubernetes-readiness-race-report.json',*sorted((root/'labs/realtime-cache').glob('endurance-*-report.json'))]:
