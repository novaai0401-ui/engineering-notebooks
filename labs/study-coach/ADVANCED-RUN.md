...
The full-stack model test sets a 200-second lease and a 195-second worker deadline to accommodate local inference before the first published answer. The buffered adapter has a 180-second overall deadline; the model runner uses a 190-second read timeout, a 195-second worker deadline and a 200-second lease. Do not enable model mode with the short extractive-default timeouts. This is intentionally different from the short extractive default. Independent lease heartbeats are a useful further production extension; increasing a lease also increases recovery delay after a crash.
...
Native Prometheus scraping and an alert fixture are exercised by the monitoring harness, which also writes a saved dashboard. The separate `labs/collector-lab/run.py` now downloads/verifies and runs a native OTLP collector, checks a real exported span and stops it. Local Docker/Kubernetes verification is recorded separately; public-cloud deployment remains unexecuted. Consult the generated validation report for exact passing checks and remaining limitations rather than treating the existence of a file as proof of deployment.

### Buffered generation verification

The recorded Java → Python → Ollama run completed in 39 seconds and correctly marked the answer as requiring factual review. It passed transport and publication checks. **It failed semantic review:** its answer implied that a checkpoint ensures idempotent side effects. A checkpoint saves workflow state; safe repeated effects require separate deduplication or transactional controls. See `live-stack-buffered-report.json`. This does not improve the broader factual evaluation scores.
