...
import base64,json,os,re,secrets,socket,subprocess,sys,threading,time
...
processes=[];logs=[];proxy=None;blocked=True
...
try:
 ...
 for port in (8091,8096):
  p=start([java,'-Xmx192m','-jar',ROOT/'java/target/study-coach-1.0.0.jar'],ROOT/'java',str(port),{'SERVER_PORT':str(port),'COACH_DB':'jdbc:h2:file:'+str(source/('app-'+str(port))).replace('\\','/')});ready(f'http://127.0.0.1:{port}/health',p)
 ...
 blocked=False;wait(lambda:health()['pending']==0);assert health()['expired']==0
 ...
 (ROOT/'durable-spring-report.json').write_text(json.dumps(report,indent=2),encoding='utf-8');print(json.dumps(report,indent=2))
